...
SLAVE_UTILIZATION=0
for i in $($APP_ROOT/bin/cluster run -q -r Slave 'pgrep -cf java.*Standalone')
do
    SLAVE_UTILIZATION=$(( $SLAVE_UTILIZATION + $i))
done
...
echo "Slave Utilization: $SLAVE_UTILIZATION/$SLAVE_COUNT"
