...
echo "- Cluster MasterScalingGroup: $(aws cloudformation list-stack-resources --stack-name=$CFN_CLUSTER_NAME | $APP_ROOT/lib/jsawk -n 'for (var i = 0; i < this.StackResourceSummaries.length; i++) { if (this.StackResourceSummaries[i].LogicalResourceId == "MasterScalingGroup") { out(this.StackResourceSummaries[i].PhysicalResourceId); } }')"
echo "- Cluster SlaveScalingGroup: $(aws cloudformation list-stack-resources --stack-name=$CFN_CLUSTER_NAME | $APP_ROOT/lib/jsawk -n 'for (var i = 0; i < this.StackResourceSummaries.length; i++) { if (this.StackResourceSummaries[i].LogicalResourceId == "SlaveScalingGroup") { out(this.StackResourceSummaries[i].PhysicalResourceId); } }')"
echo ""
echo "- Master"
